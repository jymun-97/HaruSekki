...
@Singleton
class LoadAllSearchKeywordsUseCase @Inject constructor(
    private val dispatcherProvider: DispatcherProvider,
    private val searchKeywordRepository: SearchKeywordRepository
) {

    suspend operator fun invoke(): List<SearchKeyword> = withContext(dispatcherProvider.io) {

        return@withContext searchKeywordRepository.loadAllKeywords().map {
            SearchKeyword(
                id = it.id,
                type = ModelType.SEARCH_KEYWORD,
                keyword = it.keyword
            )
        }
    }
